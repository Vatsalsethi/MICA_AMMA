	...
	titanic_train_mean_vatsal <- titanic_train_start
	titanic_train_mean_vatsal2 <- titanic_train_start
	titanic_train_mean_vatsal$Age[is.na(titanic_train_mean_vatsal$Age)] = mean(titanic_train_mean_vatsal$Age, na.rm = TRUE)
	titanic_train_mean_vatsal2$Age[is.na(titanic_train_mean_vatsal2$Age)] = mean(titanic_train_mean_vatsal2$Age, na.rm = TRUE)
	...
	full.model.titanic.mean <- glm(formula = Survived ~ Pclass + Sex + SibSp + Parch + Fare + Age,
	                               data=titanic_train_mean_vatsal, family = binomial) #family = binomial implies that the type of regression is logistic
	

	#lm
	fit.train.mean <- lm(formula = Survived ~ Pclass + Sex + SibSp + Parch + Fare + Age,
	                     data=titanic_train_mean_vatsal2) #family = binomial implies that the type of regression is logistic
	...
	titanic_train_mean_vatsal$prob = predict(full.model.titanic.mean, type=c("response"))
	titanic_train_mean_vatsal$Survived.pred = ifelse(titanic_train_mean_vatsal$prob>=.5,'pred_yes','pred_no')
	table(titanic_train_mean_vatsal$Survived.pred,titanic_train_mean_vatsal$Survived)
	

	#Testing performance on test set
	nrow(titanic_test)
	nrow(titanic_test2_mean_vatsal)
	titanic_test2_mean_vatsal <- titanic_test_start
	

	#imputation by replacing NAs by means in the test set
	titanic_test2_mean_vatsal$Age[is.na(titanic_test2_mean_vatsal$Age)] = mean(titanic_test2_mean_vatsal$Age, na.rm = TRUE)
	

	titanic_test2_mean_vatsal$prob = predict(full.model.titanic.mean, newdata=titanic_test2_mean_vatsal, type=c("response"))
	titanic_test2_mean_vatsal$Survived.pred = ifelse(titanic_test2_mean_vatsal$prob>=.5,'pred_yes','pred_no')
	table(titanic_test2_mean_vatsal$Survived.pred,titanic_test2_mean_vatsal$Survived)
	...
	titanic_train_mean_vatsal$decile <- with(titanic_train_mean_vatsal, cut(prob, 
	                                                                          breaks=quantile(prob, probs=seq(0,1, by=0.1), na.rm=TRUE), 
	                                                                          include.lowest=TRUE))
	titanic_train_mean_vatsal$decile <- as.numeric(titanic_train_mean_vatsal$decile)
	titanic_train_mean_vatsal <- titanic_train_mean_vatsal[order(-titanic_train_mean_vatsal$decile),]
	titanic.train.mean.group <- titanic_train_mean_vatsal %>% group_by(decile)
	...
	decile.titanic.mean <- aggregate(titanic_train_mean_vatsal$Survived, by=list(titanic_train_mean_vatsal$decile), FUN = sum)
	...
	Kfoldobj <- Kfold_func(titanic_train_mean_vatsal,Survived ~ Pclass + Sex + SibSp + Age,binomial,10)
	

	#Calling the Mean Squared Error function on the training set here
	MSE_Train <-MeanSquareError_func(titanic_train_mean_vatsal,Survived ~ Pclass + Sex + SibSp + Age)
	

	#confusion matrix on training set
	table(titanic_train_mean_vatsal$Survived,round(Kfoldobj$cvhat))
	print("Estimate of Accuracy")
	print(Kfoldobj$acc.cv)
	

	#Performing KFold CV on test set by calling the KFOLD CV function here
	Kfoldobj.test <- Kfold_func(titanic_test2_mean_vatsal,Survived ~ Pclass + Sex + SibSp + Age,binomial,10)
	

	#Calling the Mean Squared Error function on the test set here
	MSE_Test <-MeanSquareError_func(titanic_test2_mean_vatsal,Survived ~ Pclass + Sex + SibSp + Age)
	

	#Confusion matrix on test set
	table(titanic_test2_mean_vatsal$Survived,round(Kfoldobj.test$cvhat))
	print("Estimate of Accuracy")
